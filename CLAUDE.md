...
## Deploy
- **Frontend:** `git push origin main` → **Vercel** reconstrói (~1-2 min) → hard refresh (Cmd+Shift+R). App Vercel: `aikortex.vercel.app` (migrando do Lovable; DNS `app.aikortex.com` a virar). Sem `package-lock.json` (Vercel faz fresh install). Sem deps do Lovable.
...
- `HTTP-Referer` sempre: `https://app.aikortex.com`
