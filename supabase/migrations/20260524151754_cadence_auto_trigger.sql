-- Sprint 2.7-d: Auto-trigger de cadências
-- Quando uma row é inserida em client_table_rows, dispara automaticamente
-- todas as cadências configuradas com trigger_type='auto' apontando pra
-- aquela tabela.
...
-- Função trigger: para cada row inserida, varre cadências auto-armadas
-- nessa tabela e cria uma cadence_executions pronta pra rodar.
--
-- Cuidados:
--   - SECURITY DEFINER pra poder gravar em cadence_executions independente do caller
--   - EXCEPTION block silencioso pra NUNCA bloquear o INSERT da row original
--   - Extrai contato (nome/telefone/email) com fallbacks pra chaves comuns em PT/EN
...
BEGIN
  -- Extrai contato uma vez (reusa pra cada cadência disparada)
  ...
  LOOP
    BEGIN
      ...
      INSERT INTO public.cadence_executions (
        ...
        'pending',
        now(),
        now() + (v_delay_sec || ' seconds')::interval,
        jsonb_build_object(
          'auto_triggered', true,
    ...
    EXCEPTION WHEN OTHERS THEN
      -- Loga silenciosamente; NUNCA bloqueia o INSERT da row original
