...
const AuthModal = ({ open, mode = "signin", onClose, dismissible = true }: AuthModalProps) => {
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center ${dismissible ? "bg-black/60 backdrop-blur-sm" : ""}`}>
      <div className="relative w-full max-w-md mx-4 rounded-2xl border border-white/10 bg-[#0f1119] p-8 shadow-2xl">
        ...
        <h2 className="text-2xl font-semibold text-white text-center mb-2">
          {isSignUp ? "Comece a criar com Aikortex" : "Bem-vindo de volta"}
        </h2>
        ...
        <div className="space-y-3 mb-6">
          <button
            ...
            className="w-full h-12 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 text-white text-sm font-medium flex items-center justify-center gap-3 transition-colors"
          >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="relative">
            <input
              ...
              required
              minLength={6}
              className="w-full h-12 px-4 pr-12 rounded-xl border border-white/10 bg-white/5 text-white text-sm placeholder:text-white/30 outline-none focus:border-primary/50 transition-colors"
            />
            <button
              type="button"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full h-12 rounded-xl bg-primary hover:bg-primary/90 text-primary-foreground font-medium text-sm transition-colors disabled:opacity-50"
          >
        ...
        <p className="text-center text-xs text-white/25 mt-4">
          ...
          <span className="text-primary cursor-pointer hover:underline">Termos</span> e{" "}
