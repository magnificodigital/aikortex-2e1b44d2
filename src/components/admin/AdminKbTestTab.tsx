...
import { useIngestDocument } from "@/hooks/use-agent-knowledge-bases";
...
export default function AdminKbTestTab() {
  ...
  const [mode, setMode] = useState<"text" | "faq">("text");
  ...
  async function handleTestIngest() {
    ...
    try {
      ...
      const payload =
        mode === "text"
          ? { kb_id: kb.id, source_type: "text" as const, title: "Test Text", raw_content: textContent }
          : { kb_id: kb.id, source_type: "faq" as const, title: "Test FAQ", faqs };

      const ingest = await ingestMutation.mutateAsync(payload);

      // 3. Pega 1ª chunk (embedding) e roda match
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <Badge variant="outline" className="text-[10px]">Sprint 2.5-b</Badge>
      </div>
      <p className="text-sm text-muted-foreground">
        Cria uma KB descartável, ingere texto/FAQ via <code>ingest-document</code> e valida{" "}
        <code>match_kb_chunks</code> end-to-end. Remover após Sprint 2.5-d.
      </p>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Select value={agentId} onValueChange={setAgentId} disabled={loadingAgents}>
              ...
              <SelectContent>
                {agents.map((a) => (
                  <SelectItem key={a.id} value={a.id}>{a.name} <span className="text-muted-foreground text-xs ml-2">({a.id.slice(0, 8)})</span></SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>

          <Tabs value={mode} onValueChange={(v) => setMode(v as "text" | "faq")}>
            <TabsList>
              <TabsTrigger value="text">Texto livre</TabsTrigger>
              <TabsTrigger value="faq">FAQ (3 pares)</TabsTrigger>
            </TabsList>
